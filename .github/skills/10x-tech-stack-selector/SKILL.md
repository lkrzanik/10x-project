...
Ta umiejętność jest trzecim ogniwem w łańcuchu startowym (`/10x-shape → /10x-prd → 10x-tech-stack-selector → /10x-bootstrapper`). Jej jedyne zadanie: przekształcić napisany PRD w rekomendowany starter i mały, czytelny dla maszyny plik przekazania, który `/10x-bootstrapper` może odczytać, aby utworzyć szkielet projektu.

Umiejętność ta jest **ułatwiaczem decyzji w oparciu o wyselekcjonowany rejestr**, a nie generatorem rekomendacji od podstaw. Odczytuje priorytety PRD, zadaje maksymalnie ~6 dodatkowych pytań na ścieżce niestandardowej (lub skraca do sprawdzonej rekomendacji na ścieżce standardowej), analizuje karty starterów uwzględniające język w `references/starter-registry.yaml` i stosuje cztery twarde filtry jakości. Bogate uzasadnienie pozostaje w rozmowie; przekazanie pliku jest minimalne.
...
**Pomiń, gdy**: brakuje `context/foundation/prd.md` — umiejętność odmawia i przekierowuje do `/10x-shape` + `/10x-prd`. Pomiń również, gdy użytkownik jest w trakcie implementacji istniejącej bazy kodu i pyta o dodanie biblioteki lub zastąpienie pojedynczej zależności — to jest obszar `/10x-frame`, a nie wybór stosu.
...
- `project` → zasila `project_name` w pliku przekazania (zmień na kebab-case dla pliku przekazania, jeśli jeszcze nie jest w kebab-case).
...
Priorytety PRD:
  ...
  Termin:      <timeline_budget.mvp_weeks> tygodni
...
- "Popraw wartość" (Zapytam, które pole poprawić, a następnie zaktualizuję nadpisanie w pamięci (PRD na dysku pozostaje niezmienione).)
- "Zatrzymaj — najpierw napraw PRD" (Wyjdź. Uruchom ponownie /10x-prd, aby naprawić priorytety, a następnie wywołaj ponownie /10x-tech-stack-selector.)
...
<jeśli podczas wywiadu podniesiono flagę (preferencja vs jakość, brakująca
 funkcja, ostrzeżenie o tarciu podczas tworzenia szkieletu): jednowierszowe podsumowanie tego, co się pojawiło,
...
Rozwiąż `hints.deployment_target` z Q4. Jeśli użytkownik wybrał "Jeszcze nie wiem — wybierz dla mnie zalecaną wartość domyślną", użyj pierwszej wartości `deployment_default` karty (NIE dosłownego ciągu znaków `unspecified`).
...
- "Zapisz jako tech-stack-v2.md" (Zachowaj historię. Nowy wybór zostanie zapisany w następnym dostępnym slocie tech-stack-vN.md.)
