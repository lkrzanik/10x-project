...
public class ClimateImportControllerTests
{
    ...
    [Fact]
    public async Task Index_Post_WhenImportThrowsUnexpectedException_ReturnsUserFacingError()
    {
        var sut = CreateSut((_, _, _) => throw new IOException("Boom"));
        var model = new ClimateImportFormViewModel
        {
            SchemaType = CsvSchemaType.Sensor,
            File = CreateFormFile("sensor.csv", "Timestamp,Temperature,Humidity\n2026-01-01,10.4,58.0\n")
        ...
        var modelStateEntry = Assert.Single(sut.ModelState[string.Empty]!.Errors);
        Assert.Equal("Wystąpił nieoczekiwany błąd podczas importu. Spróbuj ponownie.", modelStateEntry.ErrorMessage);
    ...
    private sealed class FakeClimateImportOrchestrator : IClimateImportOrchestrator
    {
        private readonly Func<Stream, CsvSchemaType, CancellationToken, Task<CsvImportResult>> _importAsync;

        public FakeClimateImportOrchestrator(Func<Stream, CsvSchemaType, CancellationToken, Task<CsvImportResult>> importAsync)
        {
